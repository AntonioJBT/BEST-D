...
png('heatDiagram_joint_tx_pairs_21_apr.png', width = 12, height = 12, units = 'in', res = 300)
heatDiagram(results_by_pairing_all_treated, fit_all_treated_2_subset_coef)#, primary = 'treatmenttreated_4000')
dev.off()

png('vennDiagram_joint_tx_pairs_21_apr.png', width = 12, height = 12, units = 'in', res = 300)
vennDiagram(results_by_pairing_all_treated)
dev.off()

volcanoplot(fit_all_treated_2, coef=299) # joint treated
volcanoplot(fit_all_treated_2, coef=300) # placebo
png('volcano_plots_joint_tx_pairs_21_apr.png', width = 12, height = 12, units = 'in', res = 300)
par(mfrow=c(2,1))
volcanoplot(fit_all_treated_2, coef=299, highlight=0, #names=fit_by_arm_2$genes$ID, 
            xlab='Log Fold Change', ylab='Log Odds', pch=16, cex=0.35, abline(h=2), main='Paired 2000+4000')
volcanoplot(fit_all_treated_2, coef=300, highlight=0, #names=fit_by_arm_2$genes$ID, 
            xlab='Log Fold Change', ylab='Log Odds', pch=16, cex=0.35, abline(h=2), main='Paired placebo')
par(mfrow=c(1,1))
dev.off()
...
par(mfrow=c(2,1))
plot(topTable_pairing_joint_placebo$FC, -log10(topTable_pairing_joint_placebo$adj.P.Val), pch=20, main="Volcano plot: placebo", 
     abline(h=1.30103, v=c(0.95, 1.05)), ylab = '-log10 adjusted P-value', xlab='Fold change')
plot(topTable_pairing_joint_treated$FC, -log10(topTable_pairing_joint_treated$adj.P.Val), pch=20, main="Volcano plot: 1,25(OH)2D3 Tx", 
     abline(h=1.30103, v=c(0.95, 1.05)), ylab = '-log10 adjusted P-value', xlab='Fold change')
par(mfrow=c(1,1))
dev.off()
